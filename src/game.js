...
export default class Game {
  ...
  start() {
    ...
    if (this.gamestate !== this.gamestates.MENU && this.gamestate !== this.gamestates.NEWLEVEL) return;
    this.gamestate = this.gamestates.RUNNING;
    this.levels[this.currentLevel].buildLevel(this.levels[this.currentLevel].structure, this);
  ...
  update(dt) {
    ...
    if (!this.entities.find((entity) => {
      return entity instanceof RoundObstacle || entity instanceof Ball
    })) {
    ...
    } else if (this.levels[this.currentLevel].balls === 0 && !this.entities.find((entity) => {
  ...
  draw(ctx) {
    ...
    if (this.gamestate === this.gamestates.PAUSED || this.gamestate === this.gamestates.RUNNING) {

      ctx.font = "30px Arial";
      ctx.fillStyle = "Black";
      ctx.textAlign = "center";
      ctx.fillText(`Left: ${this.levels[this.currentLevel].balls}`, this.gameWidth - 60, this.gameHeight - 10);
    ...
    if (this.gamestate === this.gamestates.WIN) {
      ...
      ctx.fillStyle = "White";
      ctx.textAlign = "center";
      ctx.fillText("Congratulations, you just won the game!!!", this.gameWidth / 2, this.gameHeight / 2);
