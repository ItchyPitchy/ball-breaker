...
export function detectCollisions(gameObjects) {
  ...
  for (let i = 0; i < gameObjects.length; i++) {
    ...
    for (let j = i + 1; j < gameObjects.length; j++) {
      ...
      if (circleIntersect(obj1.position.x, obj1.position.y, obj1.radii, obj2.position.x, obj2.position.y, obj2.radii)){
        ...
        if (obj1.stationary) {
          ...
          const angle1 = Math.atan(distanceBetweenObjects.y / distanceBetweenObjects.x) - Math.PI / 2;
          ...
          if (obj1.position.x <= obj2.position.x) {
            obj2ColliPos = {x: obj1.position.x - Math.cos(angle1 - Math.PI / 2) * radiis, y: obj1.position.y - Math.sin(angle1 - Math.PI / 2) * radiis};
          } else {
            obj2ColliPos = {x: obj1.position.x + Math.cos(angle1 - Math.PI / 2) * radiis, y: obj1.position.y + Math.sin(angle1 - Math.PI / 2) * radiis};
          }
          ...
          const distanceBetweenObjects = {x: obj1.position.x - obj2.position.x, y: obj1.position.y - obj2.position.y};
          // const angle1 = (Math.atan2(distanceBetweenObjects.x, -(distanceBetweenObjects.y)) - Math.PI / 2);
          const angle1 = Math.atan(distanceBetweenObjects.y / distanceBetweenObjects.x) - Math.PI / 2;
          const nx = -(Math.sin(angle1));
          const ny = Math.cos(angle1);
          ...
          if (obj1.position.x >= obj2.position.x) {
            obj2ColliPos = {x: obj2.position.x - Math.cos(angle1 - Math.PI / 2) * radiis, y: obj2.position.y - Math.sin(angle1 - Math.PI / 2) * radiis};
          } else {
            obj2ColliPos = {x: obj2.position.x + Math.cos(angle1 - Math.PI / 2) * radiis, y: obj2.position.y + Math.sin(angle1 - Math.PI / 2) * radiis};
          }
          ...
          let distance = Math.sqrt((obj2.position.x-obj1.position.x)*(obj2.position.x-obj1.position.x) + (obj2.position.y-obj1.position.y)*(obj2.position.y-obj1.position.y));
          let vCollisionNorm = {x: vCollision.x / distance, y: vCollision.y / distance};
          let vRelativeVelocity = {x: obj1.speed.x - obj2.speed.x, y: obj1.speed.y - obj2.speed.y};
          let speed = vRelativeVelocity.x * vCollisionNorm.x + vRelativeVelocity.y * vCollisionNorm.y;
